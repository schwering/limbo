...
TEST(term_test, symbol) {
  ...
  EXPECT_TRUE(n1.name() && !n1.variable() && !n1.function());
  EXPECT_TRUE(n2.name() && !n2.variable() && !n2.function());

  const Term x1 = Term::Create(Symbol::CreateVariable(1, s1));
  const Term x2 = Term::Create(Symbol::CreateVariable(2, s1));
  EXPECT_TRUE(!x1.name() && x1.variable() && !x1.function());
  EXPECT_TRUE(!x2.name() && x2.variable() && !x2.function());
  ...
  EXPECT_TRUE(!f1.name() && !f1.variable() && f1.function() && f1.ground() && f1.primitive() && f1.quasiprimitive());
  EXPECT_TRUE(!f2.name() && !f2.variable() && f2.function() && !f2.ground() && !f2.primitive() && f2.quasiprimitive());
  EXPECT_TRUE(!f3.name() && !f3.variable() && f3.function() && f3.ground() && !f3.primitive() && !f3.quasiprimitive());
  EXPECT_TRUE(!f4.name() && !f4.variable() && f4.function() && f4.ground() && !f4.primitive() && !f4.quasiprimitive());
