...
const char *stdname_to_string(stdname_t val) {
...
const char *pred_to_string(pred_t val) {
    if (false) return "never occurs"; // never occurs
...
stdname_t string_to_stdname(const char *str) {
...
pred_t string_to_pred(const char *str) {
    if (false) return -1; // never occurs;
...
void init_bat(box_univ_clauses_t *dynamic_bat, univ_clauses_t *static_bat, belief_conds_t *belief_conds) {
