...
init_maps(Stream, StdNames, PredNames, SortNames) :-
    ...
    format(Stream, '~n', []),
    format(Stream, '  {~n', []),
    format(Stream, '    std::map<Atom::PredId, std::string>& map = pred_to_string_;~n', []),
...
define_regression_step(BodyStream, Class, [(Lhs,Rhs)|Boxes]) :-
    ...
    format(BodyStream, '    const Atom _lhs = ~w;~n', [Lhs_C]),
    format(BodyStream, '    if (_a.z().size() >= _lhs.z().size()) {~n', []),
    format(BodyStream, '      const size_t _n = _a.z().size() - _lhs.z().size();~n', []),
    format(BodyStream, '      const TermSeq _prefix(_a.z().begin(), _a.z().begin() + _n);~n', [VarsTerm]),
    format(BodyStream, '      const Atom _aa = _a.DropActions(_n);~n', [VarsTerm]),
    format(BodyStream, '      Unifier _theta;~n', []),
    format(BodyStream, '      if (_aa.Matches(_lhs, &_theta)) {~n', []),
    format(BodyStream, '        Formula::Ptr rhs(~w);~n', [Rhs_C]),
    format(BodyStream, '        rhs->SubstituteInPlace(_theta);~n', [Rhs_C]),
    format(BodyStream, '        rhs->PrependActions(_prefix);~n', [Rhs_C]),
...
compile_all(Class, Input, Header, Body) :-
    ...
    format(BodyStream, '~n', []),
    format(BodyStream, 'Maybe<Formula::Ptr> ~w::RegressOneStep(const Atom& _a) {~n', [Class]),
    define_regression_step(BodyStream, Class, Boxes),
    ...
    format(HeaderStream, '  Maybe<Formula::Ptr> RegressOneStep(const Atom& _a) override;~n', []),
    format(HeaderStream, '};~n', []),
    format(HeaderStream, '~n', []),
    format(HeaderStream, '}  // namespace bats~n', []),
    format(HeaderStream, '~n', []),
